const Config = {
    FEF_FOLDER: null,
    SRC_FOLDER: process.cwd() ,
    DIST_FOLDER: process.cwd() + '/dist',
...
const Builder = {
    ...
    archive: function (){
        ...
        var customObjectsPathsArr = [];
        let coFilesArr =  glob.sync(Config.SRC_FOLDER + '/**/$*/**/*.js');//assumption: custom object folders starting with $
        let jsFilesArr = glob.sync(Config.SRC_FOLDER + '/!(node_modules)/**/*.js');//get *.js files except in node_modules
        let commonFilesArr = _.difference(jsFilesArr,coFilesArr);//fetch only relevant common files with out in 

        Config.FEF_FOLDER = coFilesArr[0].substring(0,coFilesArr[0].indexOf('$')-1);

        Log.done('Starting FEF build process...');

        Builder.analyseCommonFiles(commonFilesArr);

        if( Builder.commonFileObjectsArr && Config.FEF_FOLDER != null){////
            ...
            fs.readdirSync(Config.FEF_FOLDER).forEach(folderName => {//iterate over custom object folders
                ...
                customObjectsPathsArr.push(coFolderPath);
        ...
        Builder.createZip().then(() => {
            ...
            Builder.removeIncludedCommonFolders(customObjectsPathsArr);
        })
        .catch(error => {
    ...
    analyseFile: function (file){
        ...
        declarations = Builder.findDeclaration(ast);
    ...
    findDeclaration: function (astBody){
        ...
        Builder.traverse(astBody, function(node){//callback of recursive function
            ...
            }else if(node.type === 'BinaryExpression' && node.right.type === 'Identifier'){
                addStatsEntry(node.right.name);
                identifiersArr[node.right.name].calls++;//check this identifier called in some where
            }else if(node.type === 'BinaryExpression' && node.left.type === 'Identifier'){
                addStatsEntry(node.left.name);
                identifiersArr[node.left.name].calls++;//check this identifier called in some where
            }
        });
        ////////
    ...
    /**
     * remove included common folders after getting archive
     * @param coFolderPathsArr folder paths of common objects
     */
